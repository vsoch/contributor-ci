...
function draw_force_graph(areaID, adjacentAreaID) {
    // URL for data
    var url1 = '{{ site.baseurl }}/{{ site.cci_data }}/cci-repo_dependencies.json';
    var url2 = '{{ site.baseurl }}/{{ site.cci_data }}/cci-dependencies.json';

    var files = [url1, url2];
    // Converts json file into object, reformats data, and then draws graph.
    Promise.all(files.map(url => d3.json(url))).then(values => drawGraph(reformatData({"data": values[0]}, {"data":values[1]}), areaID, adjacentAreaID));
