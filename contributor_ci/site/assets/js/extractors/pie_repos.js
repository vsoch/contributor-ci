...
function draw_pie_repos(areaID) {
    ...
    var url0 = '{{ site.baseurl }}/{{ site.cci_data }}/cci-repos.json';
    var url1 = '{{ site.baseurl }}/{{ site.cci_data }}/cci-external-users.json';
    var files = [url0,url1];
    var promises = [];
    ...
    Promise.all(promises).then(response => {
            var data = reformatData({"data": response[0]}, {"data": response[1]});
            drawGraph(data, areaID);
        });

    // Draw graph from data
    function drawGraph(data, areaID) {
        var graphHeader = 'Organization Repositories';
    ...
    function reformatData(objLabRepos, objExtUsers) {
        ...
        var data = [{ label: 'External Contributors', count: subTotal }, { label: 'Only Organization Contributors', count: repoTotal - subTotal }];
        return data;
    }
}
