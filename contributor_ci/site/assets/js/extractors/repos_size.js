...
function repos_sizes(areaID) {
    var popularityFiles = ["{{ site.baseurl }}/{{ site.cci_data }}/cci-repos.json"];
    var cutOffSize = 10;
    Promise.all(popularityFiles.map(url => d3.json(url))).then(values => {
        mostPopularRepositories = generate_popularRepos({"data": values[0]}, cutOffSize);
    }).then(() => {
        draw_cluster(areaID);
    });
}
...
function draw_cluster(areaID) {
    ...
    var url = '{{ site.baseurl }}/{{ site.cci_data }}/cci-repos.json';
    var files = [url];
