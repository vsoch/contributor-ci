...
function draw_line_repo_activity(areaID, repoNameWOwner) {

    // load data file, process data, and draw visualization
    var url0 = '{{ site.baseurl }}/{{ site.cci_data }}/cci-activity_commits.json';
    var url1 = '{{ site.baseurl }}/{{ site.cci_data }}/cci-repos.json';

    var files = [url0, url1];
    Promise.all(files.map(url => d3.json(url))).then(values => {
        var data = reformatData({"data": values[0]}, {"data": values[1]});
